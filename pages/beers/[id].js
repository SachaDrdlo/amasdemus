...
export async function getServerSideProps(context) {
    ...
    const beerDataRes = await fetch(`${process.env.API_PATH}singleBeer.php?id=${query}`)
    const beerData = await beerDataRes.json()

    const beerId = beerData.id
    const beerType = encodeURI(beerData.type)
    const sameTypeRes = await fetch(`${process.env.API_PATH}singleBeerSelectOtherBeers.php?id=${beerId}&selection="${beerType}"`)
